...
- **为什么 SW128 对 BF16/FP16**：K-major 时连续 8 个 fp16 = 16B，配 128B swizzle 周期能让 4 warp 的 LDSM 完全分散到不同 bank。SW64 周期太短，FP8 才用。
- **ncu 验证**：`l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ldsm` 应为 0
- **常见踩坑**：把 `partition_D` 喂的 sA 包过 `as_position_independent_swizzle_tensor`，否则 cp.async 写进去的物理位置和 WGMMA 读取的 swizzled 视图对不上 → 全是错的数
...
- **smem 占用公式**：`STAGES × (BLK_M × BLK_K + BLK_N × BLK_K) × sizeof(T) + 2 × STAGES × 8`
  ...
  - SM90 单 block 最大 228KB → 还能放下，但接近上限
...
- **改哪里**：BLK_N 从 128 改到 256；MMA atom 从 `SM90_64x128x16_F16F16F16_SS` 换成 `SM90_64x256x16_F16F16F16_SS`
- **为什么 N 大效率高**：单条 WGMMA 算更多 FMA，指令发射开销摊得更薄；warpgroup register 用量也会变（N 越大 accumulator 越多）
- **风险**：N=256 时 accumulator 用 128 个 fp32 寄存器/线程，会逼近 SM90 上限 232/线程，可能触发 spill
- **ncu 验证**：`smsp__sass_thread_inst_executed_op_hmma_pred_on` 占比上升；同时 `launch__registers_per_thread` 不要超 240

### 4. ClusterShape `<2,1,1>` 启用 multicast（+10%）

- **改哪里**：dimCluster = {2,1,1}；TMA 用 `SM90_TMA_LOAD_MULTICAST` 加上 mcast_mask；`make_tma_atom` 传 cluster layout（参考 W6 ex14）
...
- **5060 Ti 限制**：SM120 cluster size 上限 < SM90，2x1 大概率支持，4x1 可能失败 → README 已经标注
...
参考 `include/cutlass/epilogue/fusion/sm90_callbacks_tma_warpspecialized.hpp:1-300`。
