/***************************************************************************************************
 * Ex19 — 跑通 example 48 (手动展开版)，列出 Builder 帮你推断了哪些类型
 *
 * 学习目标（先做 ex18 再做这个）：
 *   把 example 48（手动展开）与 ex18（Builder 简化）diff，把 Builder 帮你推断的类型
 *   找出来，写到本文件下方的"Builder 推断清单"里。这是真正理解 3.x 分层的最快路径——
 *   不是空读文档，而是"用户必填 vs Builder 推断"的精确边界。
 *
 * 用法：
 *   1. 复制 examples/48_hopper_warp_specialized_gemm/48_hopper_warp_specialized_gemm.cu 到本目录
 *   2. 与 ex18（Builder 版）做 diff，把"多出来的 typedef"标出来
 *   3. 跑通，正确性 PASSED
 *
 * Builder 推断清单（diff 后填空）：
 *
 *   类型                         | 在 48 里手写为                  | Builder 推断逻辑
 *   ----------------------------|--------------------------------|-----------------------------
 *   DispatchPolicy / StageCount | MainloopSm90TmaGmmaWarpSpecialized<__> | 按 smem 容量 / sizeof(Tile)
 ...
 *   MmaAtom (TiledMMA)          | SM90_64x__x16_F16F16F16_SS<__>  | 按 TileShape_MNK 反推 N
 *   ...（继续填）
 *
 * 自检（做完 ex18+ex19 后回答）：
...
int main() {
  printf("Ex19 stub: 请把 examples/48_hopper_warp_specialized_gemm/48_hopper_warp_specialized_gemm.cu\n");
