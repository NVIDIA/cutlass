...
- **源码锚点**：`include/cute/arch/tmem_allocator_sm100.hpp`（alloc/dealloc PTX）+ `include/cute/atom/mma_traits_sm100.hpp`（含 `tmem_frg` 系列，TMEM 在 atom 层的抽象）
...
- **源码锚点**：`include/cute/arch/copy_sm100_tma.hpp`（`SM100_TMA_2SM_LOAD_*D` 系列，2SM 专用 TMA）
