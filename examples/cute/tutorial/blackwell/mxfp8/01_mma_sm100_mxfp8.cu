...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
//        CuTe Tutorial for SM100 Programming -- MXFP8 x MXFP8 block-scaled variant
//
// This file is a block-scaled MXFP8 (E4M3 x E4M3 -> F32) variant of 01_mma_sm100.cu.
// It uses the cute::SM100_MMA_MXF8F6F4_SS block-scaled UMMA instruction, which reads
// A and B from SMEM (as SMEM descriptors) and consumes per-32-element scale factors
// SFA / SFB from TMEM.  The accumulator also lives in TMEM and is loaded to registers
// in the epilogue.
//
///////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            Alpha alpha, Beta beta)
{
  ...
  // Shape them after the SMEM layout atoms so that the main-loop slicing over
  // the K mode lines up with the per-instruction MMA K blocks.
...
// SFA/SFB are indexed by element coordinates (m,k) or (n,k); their layouts use
// stride-0 broadcast across the 32-element K chunks so SFA(m,k) automatically
// returns the scale factor for the chunk containing element k.
...
int main(int argc, char** argv)
{
  ...
  // Random UE8M0 scale factors with biased exponent in [125, 130]  (i.e. scale in
  // { 2^-2, 2^-1, 2^0, 2^1, 2^2, 2^3 }).  Keep the dynamic range small so fp32
  // accumulation doesn't overflow and so intrinsic fp8 quantization error stays
  // the dominant error term (not scale-induced range blow-up).
  srand(0xC0FFEE);
  auto random_ue8m0 = []() {
    int exp = 122 + (rand() % 11);  // [122, 132]  -> scale in [2^-5, 2^5]
  ...
  // Block-scaled reference: applies SFA / SFB on the host.
  ...
  // With random E4M3 inputs and random UE8M0 scales, expect a small numerical
  // error from fp8 quantization and non-associativity of fp32 accumulation.
