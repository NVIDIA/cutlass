...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
//  CuTe Tutorial for SM100 Programming -- MXFP8 x MXFP8 block-scaled variant (Multicast TMA)
//
// This file is a block-scaled MXFP8 (E4M3 x E4M3 -> F32) variant of 03_mma_tma_multicast_sm100.cu.
// Compared to 02_mma_tma_sm100_mxfp8.cu, it introduces a ClusterShape (4x4x1) and uses
// TMA multicast loads for A (along N) and B (along M). SFA / SFB are small, so each CTA
// still loads its own tile via cooperative_copy and then stages to TMEM via
// UTCCP (SM100_UTCCP_4x32dp128bit_1cta) before feeding the block-scaled UMMA.
//
///////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            Alpha alpha, Beta beta)
{
  ...
  // --- TMA multicast partitioning for A / B ---------------------------------
  // Loads of A are multicasted along the N dimension of the cluster.
                                 ...
                                 Alpha const alpha, Beta const beta)
{
  ...
  // TMA descriptors for A and B with multicast.
  Copy_Atom tma_atom_A = make_tma_atom(
      SM90_TMA_LOAD_MULTICAST{},
      mA,
      sA_layout,
      select<0,2>(mma_tiler),
      size<2>(cluster_layout_vmnk));   // number of CTAs in N-mode of cluster
  ...
  Copy_Atom tma_atom_B = make_tma_atom(
      ...
      size<1>(cluster_layout_vmnk));   // number of CTAs in M-mode of cluster
