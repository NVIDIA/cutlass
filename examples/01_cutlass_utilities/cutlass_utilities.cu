...
cudaError_t cutlass_hgemm_nn(
  ...
  using tWarpShape = typename Gemm::WarpShape;
  using tThreadBlockShape = typename Gemm::ThreadblockShape;

  printf("ThreadblockShape<%d,%d,%d> WarpShape<%d,%d,%d>\n",  \
        tThreadBlockShape::kM, tThreadBlockShape::kN, tThreadBlockShape::kK,  \
        tWarpShape::kM, tWarpShape::kN, tWarpShape::kK);
  Gemm gemm_op;
...
cudaError_t TestCutlassGemm(int M, int N, int K, cutlass::half_t alpha, cutlass::half_t beta) {
  ...
  cutlass::HostTensor<cutlass::half_t, cutlass::layout::RowMajor> rref(cutlass::MatrixCoord(M, M));
  //
