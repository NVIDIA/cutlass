...
from .smem import SmemAllocator, SmemPartition

if TYPE_CHECKING:
    # Deferred to call sites at runtime: importing cutlass.pipeline here would
    # be circular, since cutlass.pipeline needs cutlass.memory.smem at import time.
...
class TmemBufferPool:
    ...
    def __new_from_mlir_values__(self, values: list[ir.Value]) -> "TmemBufferPool":
        ...
        pool = type(self)(new_base_ptr, self._total_cols)
...
class TmemAllocator:
    ...
    def check_valid_num_columns(self, num_columns: int) -> bool:
        ...
        if const_expr(
            ...
            and not (
                num_columns > 512
                and base_dsl.Arch.from_string(self._arch)
                in (
