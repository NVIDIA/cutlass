...
# Canonical implementation lives in tree_utils (also re-exported from the package
# __init__); imported here so leaf_utils does not carry a divergent copy.
from .tree_utils import is_frozen_dataclass
...
def _write_into_parent(parent: Any, key: Any, key_type: str, value: Any) -> None:
    """Write ``value`` into ``parent`` at ``key`` per ``key_type``.

    ``"attr"`` -> setattr; ``"list"``/``"dict"`` -> subscript assignment. No-op
    when ``parent`` is None. Callers previously inlined this dispatch at five
    sites (proxy install, replacement, reconstruction).
    """
    if parent is None:
        return
    if key_type == "attr":
        setattr(parent, key, value)
    elif key_type == "list" or key_type == "dict":
        parent[key] = value
...
class LeafInfo:
    ...
    def set_ir_values(self, new_vals: list[ir.Value]) -> None:
        ...
        if _is_dynamic_expression(self.obj):
            ...
            replaced = self.parent is not None and self.key_type in (
                "attr",
                "list",
                "dict",
            )
            _write_into_parent(self.parent, self.key, self.key_type, new_obj)
        ...
        # Case 3: raw ir.Value, including subclasses like ArithValue.
        if isinstance(self.obj, ir.Value):
            if len(new_vals) >= 1:
    ...
    def set_ir_value(self, new_val: ir.Value) -> None:
        """Set one IR value, preserving the legacy ``LeafInfo`` API."""
    ...
    def _replace_in_parent(self, new_val: ir.Value) -> None:
        ...
        if self.parent is None:
            ...
            return
        _write_into_parent(self.parent, self.key, self.key_type, new_val)
...
def inject_leaves(
    ...
    if immutable_proxies:
        for original_obj, proxy, parent, key, key_type in reversed(immutable_proxies):
            ...
            _write_into_parent(parent, key, key_type, new_obj)
