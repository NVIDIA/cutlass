...
from typing import Callable, Iterator, Optional, overload, List
...
def loop_selector(
    ...
    *,
    ...
    log().debug(
        "start [%s] stop [%s] step [%s] write_args [%s] full_write_args_count [%s] write_args_names [%s] unroll [%s] unroll_full [%s] prefetch_stages [%s] vectorize [%s]",
        start,
        stop,
        step,
        write_args,
...
class range:
    ...
    @overload
    def __new__(
        cls, stop, unroll=0, unroll_full=False, prefetch_stages=None, vectorize=None
    ):
        pass
...
class FormattedValue:
    ...
    value: Any
    conversion: int = -1
    format_spec: Optional[List[str]] = None
    ...
    def to_str(self):
        ...
        """
        if executor._is_dynamic_expression(self.value):
...
def fstring_decompose(joinedStrComponent):
    """
